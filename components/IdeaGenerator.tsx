...
export const IdeaGenerator: React.FC<IdeaGeneratorProps> = ({ 
  ...
  const handleDownload = async (idea: AppProduct) => {
      try {
        ...
        const safeName = idea.name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
        const rootFolder = zip.folder(safeName);

        if (!rootFolder) return;

        // 2. Add README.md
        const readmeContent = `# ${idea.name}\n\n> ${idea.tagline}\n\n## Overview\n${idea.description}\n\n## Target Audience\n${idea.targetAudience}\n\n## Tech Stack\n${idea.techStack.join(', ')}\n\n## Features\n${idea.features.map(f => `- ${f}`).join('\n')}\n\n## Generated by RenderBot AI`;
        rootFolder.file("README.md", readmeContent);

        // 3. Add package.json (Mock)
        const packageJson = {
          name: safeName,
          version: "0.1.0",
          private: true,
        ...
        // 4. Add Source Code Skeleton
        const srcFolder = rootFolder.folder("src");
        if (srcFolder) {
            const isFlutter = idea.techStack.some(t => t.includes('Flutter'));
            const isPython = idea.techStack.some(t => t.includes('Python'));

            if (isFlutter) {
                 srcFolder.file("main.dart", `import 'package:flutter/material.dart';\n\nvoid main() {\n  runApp(const MyApp());\n}\n\nclass MyApp extends StatelessWidget {\n  const MyApp({super.key});\n\n  @override\n  Widget build(BuildContext context) {\n    return MaterialApp(\n      title: '${idea.name}',\n      home: Scaffold(\n        appBar: AppBar(title: const Text('${idea.name}')),\n        body: const Center(child: Text('${idea.tagline}')),\n      ),\n    );\n  }\n}`);
            } else if (isPython) {
                 srcFolder.file("main.py", `def main():\n    print("Starting ${idea.name}...")\n    print("Features loaded: ${idea.features.length}")\n\nif __name__ == "__main__":\n    main()`);
            } else {
                 // Default React/JS
        ...
        link.href = url;
        link.setAttribute('download', `${safeName}-source.zip`);
